...
- Cabin floor projection은 `src/frontend/src/utils/cabinProjection.ts`에 중앙화합니다. Phaser stage는 `GET /api/v1/game/state`에서 받은 backend cabin grid 계약과 placement를 props로 받아 floor 위에 visible isometric debug grid를 그리고, `default.octocat` 펫로그가 배치되면 임시 animated placeholder를 표시합니다. 배치 대기 중에는 held placeholder를 마우스에 붙이고, floor click 좌표를 backend grid coordinate로 역변환합니다. Debug overlay는 vertical placement 높이를 확인할 수 있도록 일부 기준점에 `z=1..3` guide marker도 표시합니다.
