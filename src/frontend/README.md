...
- Cabin floor projection is centralized in `src/frontend/src/utils/cabinProjection.ts`. The Phaser stage receives the backend cabin grid contract and placements from `GET /api/v1/game/state`, draws a visible isometric debug grid over the floor, renders a temporary animated `default.octocat` placeholder when that pet log is placed, and converts placement clicks back into backend grid coordinates. The debug overlay also draws a few `z=1..3` guide markers to verify vertical placement height.
