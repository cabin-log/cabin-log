...
- On the first cabin visit per local user and settled reward date, `/cabin` calls `POST /api/v1/game/rewards/sync` once, then reloads `GET /api/v1/game/state`. The backend settles the last completed daily window plus any one-time GitHub history onboarding package, and the HUD refresh button repeats the same sync manually.
- Package cards can be claimed in place with `POST /api/v1/rewards/packages/{package_id}/claim`; the cabin state is reloaded afterward so the inventory and collection immediately reflect the new rewards. Inventory is split into supplies, furniture, and pet logs, while collection only tracks furniture and pet logs.
...
- Cabin floor projection is centralized in `src/frontend/src/utils/cabinProjection.ts`. The Phaser stage receives the backend cabin grid contract from `GET /api/v1/game/state` and draws a visible isometric debug grid over the floor so placement coordinates can be checked against the cabin artwork before object and pet placement is added. The debug overlay also draws a few `z=1..3` guide markers to verify vertical placement height.
